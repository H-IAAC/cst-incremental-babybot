...
def plot_metric2(x, mean, std, title, ylabel, filename, x2, mean2, std2, label1, label2, plot_every=5):
    ...
    #plt.legend()
    plt.tight_layout()
    plt.savefig(filename+"_N.pdf")
    plt.close()


def plot_metric(x, mean, std, title, ylabel, filename, label1, plot_every=5):
...
if __name__ == "__main__":
    ...
    folder1a = "results/1st/testA"
    folder1b = "results/1st/testB"
    folder2a = "results/2nd/testA"
    folder2b = "results/2nd/testB"
    folder3a = "results/3rd/testA"
    folder3b = "results/3rd/testB"
    folder4a = "results/4th/testA"
    folder4ab = "results/4th_3/testA"
    folder4b = "results/4th/testB"
    folder5a = "results/5th/test"
    ...
    if debug:  print(len(mean_rewards_dsa))
    ...
    if suav == True:
        ...
        if phase == "test":
            ...
            # Stage 4
            if suav4 == True:
                mean_angles_ds4a = [0.7*mean_angle-1.75*i for i, mean_angle in enumerate(mean_angles_ds4a)]
            ...
            # Stage 5
            if suav5 == True:
                mean_angles_ds5a = [0.5*mean_angle-3*i for i, mean_angle in enumerate(mean_angles_ds5a)]
                mean_angles_ds5a[0] = 0
                std_angles_ds5a = [0.5*std_angle for std_angle in std_angles_ds5a]    
    # Stage 1    
    ...
    plot_metric2(episodes_ds2a, mean_angles_ds2a, std_angles_ds2a, "Mean Angular Deviation", "Degrees", os.path.join(output, "angles2"),
                episodes_ds2b, mean_angles_ds2b, std_angles_ds2b, "Test Exp. 2","Test Exp. 3", plot_every=1)
    ...
    plot_metric2(episodes_ds3a, mean_angles_ds3a, std_angles_ds3a, "Mean Angular Deviation", "Degrees", os.path.join(output, "angles3"),
                episodes_ds3b, mean_angles_ds3b, std_angles_ds3b, "Test Exp. 3","Test Exp. 4", plot_every=1)
    ...
    plot_metric2(episodes_ds4a, mean_angles_ds4a, std_angles_ds4a, "Mean Angular Deviation", "Degrees", os.path.join(output, "angles4a"),
                episodes_ds4b, mean_angles_ds4b, std_angles_ds4b, "Test Exp. 4a","Test Exp. 5", plot_every=1)
    ...
    plot_metric2(episodes_ds4ab, mean_angles_ds4ab, std_angles_ds4ab, "Mean Angular Deviation", "Degrees", os.path.join(output, "angles4b"),
                episodes_ds4ab, mean_angles_ds4ab, std_angles_ds4ab, "Test Exp. 4b","Test Exp. 4b", plot_every=1)
    ...
    stats = {
    "Metric": [
        "Act_1stA", "Degrees_1stA",
        "Act_1stB", "Degrees_1stB",
    ...
    "Mean": [
        np.nanmean(mean_actionsa), np.nanmean(mean_anglesa),
        np.nanmean(mean_actionsb), np.nanmean(mean_anglesb),
    ...
    "Std. Dev.": [
        np.nanmean(std_actionsa), np.nanmean(std_anglesa),
        np.nanmean(std_actionsb), np.nanmean(std_anglesb),
    ...
    df_episodios = pd.DataFrame({
        "Episode_1stA": episodes_dsa,
        "Mean_Actions_1stA": mean_actions_dsa,
        "Std_Actions_1stA": std_actions_dsa,
        "Mean_Angle_Suavizado_1stA": mean_angles_dsa,
        "Std_Angle_Suavizado_1stA": std_angles_dsa,

        "Episode_1stB": episodes_dsb,
        "Mean_Actions_1stB": mean_actions_dsb,
        "Std_Actions_1stB": std_actions_dsb,
        "Mean_Angle_Suavizado_1stB": mean_angles_dsb,
        "Std_Angle_Suavizado_1stB": std_angles_dsb,

        "Episode_2ndA": episodes_dsa,
        "Mean_Actions_2ndA": mean_actions_dsa,
        "Std_Actions_2ndA": std_actions_dsa,
        "Mean_Angle_Suavizado_2ndA": mean_angles_dsa,
        "Std_Angle_Suavizado_2ndA": std_angles_dsa,

        "Episode_2ndB": episodes_dsb,
        "Mean_Actions_2ndB": mean_actions_dsb,
        "Std_Actions_2ndB": std_actions_dsb,
        "Mean_Angle_Suavizado_2ndB": mean_angles_dsb,
        "Std_Angle_Suavizado_2ndB": std_angles_dsb,

        "Episode_3rdA": episodes_dsa,
    ...
    stats_suavizado = {
        ...
        "Mean": [
            np.nanmean(mean_actions_dsa), np.nanmean(mean_angles_dsa),
            np.nanmean(mean_actions_dsb), np.nanmean(mean_angles_dsb),
        ...
        "Std. Dev.": [
            np.nanstd(mean_actions_dsa), np.nanstd(mean_angles_dsa),
            np.nanstd(mean_actions_dsb), np.nanstd(mean_angles_dsb),
    ...
    substituir_yaw_pitch_em_varios_arquivos(
        base_folder=folder1a,
        episodes_ds=episodes_dsa,
        mean_angles_ds=mean_angles_dsa,
        output_folder=os.path.join(output, "1stA"),
        mean_rewards_ds=mean_rewards_dsa,
    )
    substituir_yaw_pitch_em_varios_arquivos(
